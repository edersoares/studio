...
return [
    ...
    'drafts' => [
        ...
        Draft::new('User')
            ...
            ->attribute()->label('E-mail')->string('email')->unique()->fillable()->required()->min(3)->faker('email')
            ->attribute()->label('E-mail verificado')->timestamp('email_verified_at')->nullable()->cast('datetime')
            ->attribute()->label('Senha')->string('password')->fillable()->hidden()->cast('hashed')->required()->min(8)->max(48)->faker('lexify', '########')
